...
class ContextualBertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, args, dropout_prob=0.1):
        super(ContextualBertForVLTasks, self).__init__(config)
        ...
        self.fusion_method = config.fusion_method
        self.apply(self.init_weights)
        config.hidden_size = config.pooler_size #TODO: make dependent on 2* clip size
        config.num_attention_heads = 8
        self.context_transformer = nn.ModuleList([BertLayer(config) for _ in range(args.transformer_layers)])
        self.context_transformer.cuda()
        self.prediction_layer = nn.Linear(config.hidden_size, 1).cuda()
        self.add_inputs = args.add_inputs
    ...
    def forward(
        ...
        if self.fusion_method == "sum":
            ...
            raise ValueError("Invalid fusion method: %s" % self.fusion_method)
        x_ = pooled_output.reshape((pooled_output.shape[0]//10, 10, pooled_output.shape[-1]))
        if self.positional:
            embs = self.positional_emb(torch.arange(10).cuda())
            embs = torch.stack(x_.shape[0]*[embs])
            pos_mask = pos_mask.reshape((x_.shape[0], 10)).unsqueeze(dim=-1)
            embs = embs * pos_mask
            x_pos = x_ + embs
        else:
            x_pos = x_
        attention_mask = torch.ones((pooled_output.shape[0]//10,1,1,10)).cuda()
        x = self.context_transformer[0](x_pos, attention_mask)
        for layer in self.context_transformer[1:]:
            x = layer(x, attention_mask)
        if self.add_inputs:
            x = x + x_
        x = x.reshape((x.shape[0] * x.shape[1], x.shape[2]))
        vil_prediction = self.prediction_layer(x)
