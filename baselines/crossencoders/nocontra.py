...
from volta.task_utils import LoadDataset, LoadDatasetEval, LoadLoss, ForwardModelsTrain, ForwardModelsVal
import wandb
wandb.init(project='finetune-uniter', settings=wandb.Settings(start_method='fork'))
...
#        batch_size, task2num_iters, dset_val, dl_val = LoadDatasetEval(args, config, task_cfg, args.task)
        batch_size, task2num_iters, _, dset_val, _, dl_val = LoadDataset(args, config, task_cfg, args.task, "val")
        dset_train = dset_val
        dl_train = dl_val 
        task2num_iters = {'TASK3': len(dl_train)} 
    else:
        batch_size, task2num_iters, dset_train, dset_val, dl_train, dl_val, dset_test, dl_test = LoadDataset(args, config, task_cfg, args.task)
