...
from volta.encoders import ContextualBertForVLTasks
...
import wandb
wandb.init(project='finetune-contextual-uniter', notes="fixed pos emb", entity='bennokrojer', settings=wandb.Settings(start_method='fork'))
...
def parse_args():
    ...
    parser.add_argument("--only_vilbert_epochs", default=0, type=int)
    parser.add_argument("--add_inputs", action="store_true")
...
def main():
    ...
    wb_conf.add_inputs = args.add_inputs
    wb_conf.job_id = args.job_id
    ...
    # Output dirs
    if args.save_name:
        args.save_name = args.save_name + '_' + str(args.lr) + '_' + str(args.transformer_layers)
    ...
    id2name = None
    id2name = []
    id2name.append(json.load(open('/network/scratch/b/benno.krojer/dataset/counter2key_train.json', 'r')))
    id2name.append(json.load(open('/network/scratch/b/benno.krojer/dataset/counter2key_test.json', 'r')))
    id2name.append(json.load(open('/network/scratch/b/benno.krojer/dataset/counter2key_val.json', 'r')))
    ...
    # Model
    model = ContextualBertForVLTasks.from_pretrained(args.from_pretrained, config=config, task_cfg=task_cfg, task_ids=[task], args=args)
    model.add_inputs = args.add_inputs
    ...
    if args.eval_only:
        ...
        for epoch_id in tqdm(range(start_epoch, args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in tqdm(enumerate(dl_train), desc="Batch"):
                ...
                loss = ForwardModelsTrain(config, task_cfg, device, task, batch, model, criterion, id2name[0], args.all_pos)
...
#                if True:
                    score = evaluate(config, dl_val, task_cfg, device, task, criterion, model, epoch_id, default_gpu, tb_logger, args, id2name)
                    score = evaluate(config, dl_test, task_cfg, device, task, criterion, model, epoch_id, default_gpu, tb_logger, args, id2name, val=False)
                    if score > max_score:
                        max_score = score
...
def evaluate(config, dataloader_val, task_cfg, device, task_id, criterion, model, epoch_id, default_gpu, tb_logger, args, id2name, val=True):
    model.eval()
    question_ids = []
    preds = []
    for i, batch in enumerate(dataloader_val):
        ...
        loss, batch_size, vil_prediction, question_id = ForwardModelsVal(config, task_cfg, device, task_id, batch, model, criterion, id2name[2 if val else 1], args.all_pos)
